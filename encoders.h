...
ISR( INT6_vect ) {
  ...
  if( state == 1 ) {           // row 1 from table
    count_right--; // clockwise I'm more used to the romi going forward in the right direction.
                   // So when the motor is turning clockwise I choose -- instead of ++ 
  } else if( state == 2 ) {    // row 2 from table
    count_right++;// counter clockwise
